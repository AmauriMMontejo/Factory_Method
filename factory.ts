...
class Dragon implements Enemy {
    ...
    color = '#fb923c'
    attack(): Attack {
        return {emoji: '🔥', damage: 1, cooldown: 1700}
    }
}

class Mago implements Enemy {
    name = 'Mago'
    emoji = '🧙'
    color = '#fbbf24'
    attack(): Attack {
        return {emoji: '✨', damage: 1, cooldown: 1200}
    }
}
...
export class HardEnemyFactory extends EnemyFactory {
    private readonly roster: (new () => Enemy) [] = [Slime, Bat, Ghost, Dragon, Mago]
    protected createEnemy(): Enemy {
